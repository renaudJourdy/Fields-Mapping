...
{
  "counters": {
    "odometer": { "value": "number", "unit": "km" , "last_updated_at": "ISO"},
    "engine_hours": { "value": "number", "unit": "h", "last_updated_at": "ISO"}
  }
}
```
...
| `driver.hardware_key.extended_id` | BL/P3 | Navixy: `avl_io_238` (User ID) |  |
...
Compiling Computation Approach and Description for all non-object status fields from the documentation.

[3 tools called]

Table with Computation Approach and Description for all non-object status Fleeti fields:

| Fleeti Field Name | Computation Approach | Description |
| --- | --- | --- |
| `top_status_family` | Family name of highest-priority compatible status family. Evaluate status families in priority order: (1) Connectivity if compatible and code = "offline", (2) Immobilization if compatible and code = "immobilized", (3) Engine if compatible and code = "running", (4) Transit if compatible, (5) Connectivity fallback (code = "online"). Return the family name (connectivity/immobilization/engine/transit) of the selected status family. | Family name of the highest-priority status family selected for top_status. Used to identify which status family (connectivity, immobilization, engine, or transit) currently has priority based on conditional priority rules. |
| `top_status_code` | Code from highest-priority compatible status family. Evaluate status families in priority order: (1) Connectivity if compatible and code = "offline", (2) Immobilization if compatible and code = "immobilized", (3) Engine if compatible and code = "running", (4) Transit if compatible, (5) Connectivity fallback (code = "online"). Return the code value from the selected status family. | Status code from the highest-priority status family selected for top_status. Represents the actual status value (e.g., "offline", "immobilized", "running", "in_transit", "parked", "online") from the prioritized status family. |
| `top_status_last_changed_at` | Updates when top_status.code changes (not on every packet). Backend compares current top_status.code with previous value. If different, update timestamp to current ISO8601 UTC time. If same, keep previous timestamp unchanged. | ISO8601 timestamp when top_status.code last changed. Used to calculate duration of current top status state (e.g., "immobilized for 2 hours", "engine running since 10:30"). Enables timeline features and status duration calculations. |
| `statuses_connectivity_code` | online if last_updated_at < 24h, offline if ≥ 24h for vehicles/equipment/phones; 1h threshold for coldroom sites. Check asset subtype: if Site.Coldroom, use 1h threshold; otherwise use 24h threshold. Compare root-level last_updated_at (derived from msg_time) with current time. If difference < threshold → "online", else → "offline". | Connectivity status code: "online" or "offline" based on last_updated_at threshold. Indicates whether the asset has recently communicated with the Fleeti backend. Vehicles, Equipment, and Phones use 24h threshold; Coldroom Sites use 1h threshold. |
| `statuses_connectivity_compatible` | Check asset type/subtype against compatibility matrix (Connectivity: all types). Connectivity status is compatible with all asset types and subtypes. Return true for all assets. | Indicates if Connectivity status family is compatible with this asset type/subtype. Connectivity applies to all asset types (Vehicles, Equipment, Sites, Phones) regardless of subtype. Always true. |
| `statuses_connectivity_last_changed_at` | Updates when status family code changes (not on every packet). Backend compares current statuses_connectivity_code with previous value. If different, update timestamp to current ISO8601 UTC time. If same, keep previous timestamp unchanged. | ISO8601 timestamp when connectivity status code last changed. Used to calculate duration of current connectivity status (e.g., "offline for 2h", "online since yesterday"). Enables connectivity duration tracking and timeline features. |
| `statuses_immobilization_code` | immobilized if io.outputs[N] = 1, free if = 0, immobilizing/releasing based on command tracking. Check asset has immobilizer accessory (asset.accessories contains type_code="immobilizer"). Read digital output state from io.outputs[N] where N is from asset.installation.immobilizer_output_number. If output = 1 → "immobilized". If output = 0 → "free". If immobilization command sent via REST API → "immobilizing" (persists until output changes to 1 or timeout). If release command sent → "releasing" (persists until output changes to 0 or timeout). | Immobilization status code: "immobilized", "free", "immobilizing", or "releasing" based on digital output state and command tracking. Indicates whether the vehicle/equipment is currently immobilized (locked), free (unlocked), or in a transition state (immobilizing/releasing) managed by backend command tracking. |
| `statuses_immobilization_compatible` | Check asset type/subtype against compatibility matrix (Immobilization: Vehicles+Equipment.Undefined+Equipment.ElectricGenerator). Check asset.type ∈ {Vehicle (10), Equipment (20)} AND asset.subtype ∈ {Vehicle.*, Equipment.Undefined, Equipment.ElectricGenerator}. Return true if compatible, false otherwise. | Indicates if Immobilization status family is compatible with this asset type/subtype. Immobilization applies to all Vehicles and specific Equipment subtypes (Undefined, ElectricGenerator). Returns true only for compatible asset types/subtypes. |
| `statuses_immobilization_last_changed_at` | Updates when status family code changes (not on every packet). Backend compares current statuses_immobilization_code with previous value. If different, update timestamp to current ISO8601 UTC time. If same, keep previous timestamp unchanged. | ISO8601 timestamp when immobilization status code last changed. Used to calculate duration of current immobilization status (e.g., "immobilized for 3 hours", "free since 08:00"). Enables immobilization duration tracking and timeline features. |
| `statuses_engine_code` | running if ignition ON (io.inputs[N] = 1 OR can_ignition_state = true), standby if OFF. Check asset is engine status compatible. Read ignition state from multiple sources in priority: (1) io.inputs[N] where N is from asset.installation.ignition_input_number (fallback to input_1 if missing), (2) can_ignition_state, (3) vehicle.ignition.hardware, (4) vehicle.ignition.virtual. If any ignition source = true → "running". If all ignition sources = false → "standby". | Engine status code: "running" or "standby" based on ignition state. Indicates whether the engine is currently running (ignition ON) or in standby mode (ignition OFF). Uses multiple ignition sources (digital inputs, CAN signals, hardware/virtual ignition) with fallback logic. |
| `statuses_engine_compatible` | Check asset type/subtype against compatibility matrix (Engine: Equipment+Vehicles.Agricultural+Vehicles.Machine). Check asset.type = Equipment (all subtypes) OR (asset.type = Vehicle AND asset.subtype ∈ {Agricultural, Machine}). Return true if compatible, false otherwise. | Indicates if Engine status family is compatible with this asset type/subtype. Engine status applies to all Equipment subtypes and specific Vehicle subtypes (Agricultural, Machine). Returns true only for compatible asset types/subtypes. |
| `statuses_engine_last_changed_at` | Updates when status family code changes (not on every packet). Backend compares current statuses_engine_code with previous value. If different, update timestamp to current ISO8601 UTC time. If same, keep previous timestamp unchanged. | ISO8601 timestamp when engine status code last changed. Used to calculate duration of current engine status (e.g., "engine running since 10:30", "standby for 1 hour"). Enables engine runtime tracking and timeline features. |
| `statuses_transit_code` | State machine: in_transit if ignition ON OR (movement=true OR speed>0.5km/h), parked if ignition OFF OR (movement=false AND speed=0 for 3min). Implement state machine with immediate transitions (ignition-based) and duration-based transitions (movement/speed). Immediate: Ignition OFF→ON → "in_transit", Ignition ON→OFF → "parked". Duration-based: If current="in_transit" AND (movement=false OR speed=0) for 3min → "parked". If current="parked" AND (movement=true OR speed>0.5km/h) → "in_transit". Initial state: If ignition=true OR movement=true OR speed>0.5km/h → "in_transit", else if (movement=false AND speed=0) for 3min → "parked", else default="in_transit". | Transit status code: "in_transit" or "parked" based on state machine with ignition triggers and movement/speed conditions. Indicates whether the asset is currently in transit (moving) or parked (stationary). Uses immediate ignition-based transitions and duration-based movement/speed checks (3-minute threshold for parked detection, 0.5 km/h speed threshold for movement). |
| `statuses_transit_compatible` | Check asset type/subtype against compatibility matrix (Transit: Phones+Equipment.Undefined+All Vehicles). Check asset.type = Phone (all subtypes) OR asset.subtype = Equipment.Undefined OR asset.type = Vehicle (all subtypes except explicitly excluded). Excluded: Site.Undefined, Site.Coldroom, Equipment.FuelTank, Equipment.ElectricGenerator. Return true if compatible, false otherwise. | Indicates if Transit status family is compatible with this asset type/subtype. Transit status applies to Phones (all), Equipment.Undefined, and all Vehicle subtypes (except Sites and some Equipment subtypes). Returns true only for compatible asset types/subtypes. |
| `statuses_transit_last_changed_at` | Updates when status family code changes (not on every packet). Backend compares current statuses_transit_code with previous value. If different, update timestamp to current ISO8601 UTC time. If same, keep previous timestamp unchanged. | ISO8601 timestamp when transit status code last changed. Used to calculate duration of current transit status (e.g., "parked for 2 hours", "in_transit since 09:00"). Enables transit duration tracking, dwell-time calculations, and timeline features. |

**Notes:**

- All `last_changed_at` fields update only when the corresponding `code` field changes, not on every telemetry packet.
- Compatibility checks use `asset.type` and `asset.subtype` from asset metadata.
- Top status computation evaluates status families in priority order and selects the highest-priority compatible family.
- Each status family's code is computed independently using its own trigger conditions before top status selection.

Analyse des champs I/O dans le CSV et la spécification pour fournir Computation Approach et Description.

[4 tools called]

Vérification de la structure exacte dans le CSV:

[3 tools called]

Computation Approach et Description pour tous les champs I/O, basés sur la spécification et les champs provider :

## Digital Inputs

| Fleeti Field | Computation Approach | Description |
| --- | --- | --- |
| `inputs_individual_input_1` | Direct mapping from Navixy: `avl_io_1` (Digital Input 1). If only `inputs` bitmask is available, extract bit 0. Convert to boolean: 0 = false, 1 = true. | Digital input 1 raw state from GPS tracker device. Values: false = low/inactive, true = high/active. Semantic meaning (e.g., ignition cable, door sensor) is defined in `asset.installation.*` metadata. Used by transformation pipeline to compute canonical fields like `power.ignition`. |
| `inputs_individual_input_2` | Direct mapping from Navixy: `avl_io_2` (Digital Input 2). If only `din` bitmask is available, extract bit 1. Convert to boolean: 0 = false, 1 = true. | Digital input 2 raw state from GPS tracker device. Values: false = low/inactive, true = high/active. Semantic meaning defined in installation metadata. Used by transformation pipeline to compute canonical fields. |
| `inputs_individual_input_3` | Direct mapping from Navixy: `avl_io_3` (Digital Input 3). If only `din` bitmask is available, extract bit 2. Convert to boolean: 0 = false, 1 = true. | Digital input 3 raw state from GPS tracker device. Values: false = low/inactive, true = high/active. Semantic meaning defined in installation metadata. Used by transformation pipeline to compute canonical fields. |
| `inputs_individual_input_4` | Direct mapping from Navixy: `avl_io_4` (Pulse Counter Din1). If only `din` bitmask is available, extract bit 3. Convert to boolean: 0 = false, 1 = true. Note: This input also functions as a pulse counter, but the counter value resets when records are saved. | Digital input 4 raw state from GPS tracker device (also functions as pulse counter). Values: false = low/inactive, true = high/active. Pulse count functionality resets when records are saved. Semantic meaning defined in installation metadata. Used by transformation pipeline to compute canonical fields. |

## Digital Outputs

| Fleeti Field | Computation Approach | Description |
| --- | --- | --- |
| `outputs_individual_output_1` | Direct mapping from Navixy: `avl_io_179` (Digital Output 1). If only `outputs` bitmask is available, extract bit 0. Convert to boolean: 0 = false, 1 = true. | Digital output 1 raw state from GPS tracker device. Values: false = inactive, true = active. Semantic meaning (e.g., immobilizer relay) is defined in `asset.installation.*` metadata. Used for immobilization control and status computation. |
| `outputs_individual_output_2` | Direct mapping from Navixy: `avl_io_180` (Digital Output 2). If only `outputs` bitmask is available, extract bit 1. Convert to boolean: 0 = false, 1 = true. | Digital output 2 raw state from GPS tracker device. Values: false = inactive, true = active. Semantic meaning defined in installation metadata. Used for device control and status computation. |
| `outputs_individual_output_3` | Direct mapping from Navixy: `avl_io_380` (Digital Output 3). If only `outputs` bitmask is available, extract bit 2. Convert to boolean: 0 = false, 1 = true. | Digital output 3 raw state from GPS tracker device. Values: false = inactive, true = active. Semantic meaning defined in installation metadata. Used for device control and status computation. |

## Analog Inputs

| Fleeti Field | Computation Approach | Description |
| --- | --- | --- |
| `analog_individual_input_1_value` | Direct mapping from Navixy: `avl_io_9` (Analog Input 1). If only generic `adc` array is available, use `adc[0]` (first element). Value represents voltage reading in volts. | Analog input 1 voltage reading from GPS tracker device. Measured in volts (V). Semantic meaning (e.g., fuel level sensor, temperature probe) is defined in installation metadata. Used by transformation pipeline to compute canonical sensor/fluid fields. |
| `analog_individual_input_1_unit` | Static value: "V" (volts). Always set to "V" for analog voltage inputs. | Unit of measurement for analog input 1. Always "V" (volts) for voltage readings. |
| `analog_individual_input_2_value` | Direct mapping from Navixy: `avl_io_6` (Analog Input 2). If only generic `adc` array is available, use `adc[1]` (second element). Value represents voltage reading in volts. | Analog input 2 voltage reading from GPS tracker device. Measured in volts (V). Semantic meaning defined in installation metadata. Used by transformation pipeline to compute canonical sensor/fluid fields. |
| `analog_individual_input_2_unit` | Static value: "V" (volts). Always set to "V" for analog voltage inputs. | Unit of measurement for analog input 2. Always "V" (volts) for voltage readings. |

## Notes importantes

1. Tous les champs I/O sont de type "direct" (pas calculé) — ce sont des valeurs brutes du device.
2. Priority: BL (Business Logic) — utilisés en interne, pas affichés aux clients.
3. Bitmask extraction: si seul un bitmask est disponible (`din` pour inputs, `outputs` pour outputs), extraire le bit correspondant (bit 0 = input_1/output_1, bit 1 = input_2/output_2, etc.).
4. Analog array: si seul `adc` est disponible, utiliser l'index du tableau (`adc[0]` pour input_1, `adc[1]` pour input_2).
5. Semantic mapping: la signification sémantique (ignition, immobilizer, etc.) est définie dans `asset.installation.*` et utilisée par le pipeline de transformation pour calculer les champs canoniques.

Souhaitez-vous que je mette à jour le CSV avec ces valeurs ?
...
"[
  ...
  },
  {
    ...
    ""type"": ""direct"",
    ""source_type"": ""provider"",
    ""provider"": ""oem-trackunit"",
    ""field"": ""digital_input_1"",
    ""path"": ""io.digital_input_1"",
    ""description"": ""Digital Input 1 from OEM-Trackunit""
  },
  {
    ""priority"": 3,
    ""type"": ""calculated"",
