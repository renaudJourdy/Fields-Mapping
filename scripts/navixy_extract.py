"""
Populate Navixy Field Name column in Fleeti mapping CSV.
"""
...
def read_csv(path: Path) -> Tuple[list[list[str]], str]:
    """
    Try opening the CSV using encodings the file might use.
    """
...
def main() -> None:
    ...
    idx_name = header.index("Navixy Field Name")
    count = 0

    for row in rows[1:]:
        if len(row) <= max(idx_navixy, idx_name):
            continue
        value = row[idx_navixy]
        l = value.find("(")
        r = value.rfind(")")
        if l != -1 and r != -1 and l < r:
            label = value[l + 1 : r].strip()
            if label:
                row[idx_name] = label
                count += 1
